...
import '@/composables/markdown-view'
...
<template>
  <div class="chat h-full flex flex-col">
    <div ref="chatBox" class="mes flex-1 overflow-auto pb-6">
      <div v-for="(v, index) in list" :key="index" class="item" :class="v.role">
        <div class="item-main">
          <template v-if="v.value">
            <a-alert v-if="v.type === 'error'" :message="v.value" type="error" show-icon />
            <template v-else>
              <div v-if="v.role === 'user'" style="white-space: pre-wrap" v-html="v.value"></div>
              <markdown-view v-else :markdown="v.value"></markdown-view>
            </template>
          </template>
          <template v-else>
